...
for (category, desc), group in grouped:
    ...
    # Just count which model is better based on MAE (lower is better)
    if res['arima'] and res['holtwinters']:
        if res['arima']['mae'] < res['holtwinters']['mae']:
            arima_better_count += 1
        else:
            holtwinters_better_count += 1
    elif res['arima']:
        arima_better_count += 1
    elif res['holtwinters']:
        holtwinters_better_count += 1
...
# After loop, print the counts
print(f"\nNumber of sectors where ARIMA is better (lower MAE): {arima_better_count}")
print(f"Number of sectors where Holt-Winters is better (lower MAE): {holtwinters_better_count}")
